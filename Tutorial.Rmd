...
```{r}
scat23 <- ggplot(df, aes(x=year, y=hwdensity)) +
  geom_point(aes(col=provname), size=1) + 
  geom_smooth(method="loess", se=T, size=1) + 
...
```{r}
scat233 <- ggplot(df, aes(x=year, y=midwifedensity)) +
  geom_point(aes(col=provname), size=1) + 
  geom_smooth(method="loess", se=T, size=1) + 
  labs(title="Mozambique trends on MId-wife Density in Provinces", 
       y="Density of Mis-wife in the Region (per 1000 live births)", 
       x="Year", 
       caption = "Source: Demographic Healh Surveys") +
    theme(legend.position = "right",
  plot.caption = element_text(hjust = 0), 
